...
if($now > 20) {
	...
	//echo "SELECT * FROM `custom_show_schedule`  WHERE start_time >= '$now:00' AND show_date >= '$today' AND archived = 0 GROUP BY start_time ORDER BY start_time LIMIT 0, 4  ";
	$q = mysql_query("SELECT * FROM `custom_show_schedule`  WHERE start_time >= '$now:00' AND show_date >= '$today' AND archived = 0 GROUP BY start_time ORDER BY start_time LIMIT 0, 4  ");
}
...
$q = mysql_query("SELECT * FROM `custom_show_schedule`  WHERE start_time >= '20:00' AND start_time < '24:00' AND show_date = '$today' AND archived = 0 GROUP BY start_time ORDER BY start_time LIMIT 0, 4 ");
while($show = mysql_fetch_assoc($q)) {
	if($c % 2) {
		$even = '';
	} else {
...
$start = 20;
//echo "SELECT * FROM `custom_show_schedule`  WHERE start_time >= '20:00' AND start_time < '24:00' AND show_date = '$today' AND archived = 0 GROUP BY start_time ORDER BY start_time LIMIT 0, 4 ";
$q = mysql_query("SELECT *, CASE start_time WHEN '00:00' THEN '24:00' WHEN '00:30' THEN '24:30' WHEN '01:00' THEN '25:00' WHEN '01:30' THEN '25:30' WHEN '02:00' THEN '26:00' WHEN '02:30' THEN '26:30' WHEN '03:00' THEN '27:00' WHEN '03:30' THEN '27:30' ELSE start_time END AS mod_time FROM `custom_show_schedule`  WHERE (start_time >= '23:00' OR start_time < '04:00') AND show_date = '$today' AND archived = 0 GROUP BY start_time ORDER BY mod_time LIMIT 0, 4 ");
while($show = mysql_fetch_assoc($q)) {
	if($c % 2) {
		$even = '';
	} else {
